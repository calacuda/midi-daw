...
_new-tmux-dev-session SESSION:
  ...
  @just _new-window "{{SESSION}}" "Data Types" "cd midi-daw-types && nv src/lib.rs src/v2.rs src/v2/{*,**/*}.rs"
  ...
  @just _new-window "{{SESSION}}" "Edit Tests" ". ./test-files/.venv/bin/activate && cd midi-daw-types && nv ./v2-test-files/*.py"
  @just _new-window "{{SESSION}}" "Run Tests" ". ./test-files/.venv/bin/activate && cd midi-daw-types"
