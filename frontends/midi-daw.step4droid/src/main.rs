    ...
    rsx! {
        ...
        div {
            ...
            div {
                ...
                PianoDisplay { sections, displaying_uuid, counter, playing_sections }

                StepButtons { sections, displaying_uuid, counter, playing_sections }
            }
        }
...
fn PianoDisplay(
    ...
    rsx! {
        div {
            ...
            // "piano-display"
            for octave in 0..7 {
                PianoOctave { sections, displaying_uuid, octave, counter, playing_sections }
            }
        }
    }
}
        ...
        div {
            ...
            div {
                ...
                for note_offset in [0, 2, 4, 5, 7, 9, 11] {
                    WhiteKey {sections, displaying_uuid, octave, note_offset, step_i: counter, playing_sections}
                }
            }
            ...
            div {
                ...
                for note_offset in 0..12 {
                    BlackKey {sections, displaying_uuid, octave, note_offset, step_i: counter, playing_sections}
                }
    ...
    rsx! {
        div {
            class: {
                ...
                if selected() && playing_sections.read().contains(&displaying_uuid.read().read().map(|dis| *dis).unwrap_or(0)) {
        ...
        div {
            ...
            div {
                ...
                for (i, section) in seq_names.iter().enumerate() {
                    div {
                        ...
                        div {
                            class: "play-icon text-green",

                            if playing_sections.read().contains(&i) {
                                "|>"
                            }
                        }
            ...
            div {
                ...
                div {
                    ...
                    for i in 0..8 {
                        StepButton { sections, displaying_uuid, i, counter, playing_sections }
                    }
                }
...
fn StepButton(
    ...
    let playing = use_memo(move || {
        displaying_uuid
            .read()
            .read()
            .map(|dis| playing_sections().contains(&dis))
            .ok()
            .unwrap_or(false)
    });
    let selected = use_memo(move || {
        displaying_uuid
            ...
            .map(|dis| {
                ...
                sections
                    .read()
                    .read()
                    .map(|gaurd| counter() % gaurd.clone()[dis].steps.len() == i)
                    .ok()
            })
            .ok()
            .flatten()
    ...
    let mut blip = use_signal(|| false);

    if i == 0 {
        spawn(async move {
            let loc_blip = *blip.read();
    ...
    rsx! {
        div {
            ...
            div {
                class: {
                    let mut class = "led super-center".to_string();

                    if (playing() && selected()) || (!playing() && blip()) {
                        class.push_str(" led-on");
                    }
